...
class WebDriver(object):
    ...
    def find_element_by_selector(self, selector):
        xpath = CSSSelector(selector).path.replace("descendant-or-self::", "//")
    ...
    def get_dom(self):
        """Gets the page source as a lxml.html element."""
        return html.fromstring(self.get_page_source())
    ...
    def get_dom_when_ready(self, timeout=10):
        for index in range(timeout * 10):
            time.sleep(0.1)
            if '</html>' in self.get_page_source().lower():
                return self.get_dom()

        raise AssertionError('the page did not load within %d seconds' % timeout)
